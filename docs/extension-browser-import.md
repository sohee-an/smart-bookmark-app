...
| 케이스                              | 처리 방식                        |
| ----------------------------------- | -------------------------------- |
| 트리 내 중복 (같은 URL이 여러 폴더) | URL 기준 첫 번째만 남기고 스킵   |
| DB 기존 북마크와 중복               | 스킵 (기존 데이터 건드리지 않음) |

서버는 insert 전 `user_id + url` 조합 존재 여부를 확인하고, 이미 있으면 저장하지 않고 성공으로 카운트한다.
...
트리에서 항목 선택
    → "선택한 N개 바로 가져오기" 클릭
    → 트리 내 중복 URL 제거 (client)
    → 배치 전송 → 서버에서 DB 중복 스킵 후 저장
    → 진행률 표시 → 완료
```

---

### 경로 B — AI 정리 후 가져오기 (신규)

```
트리에서 항목 선택
    → "AI로 정리해서 가져오기" 클릭
    → 트리 내 중복 URL 제거 (client)
    → POST /api/extension/organize (url + title 목록 전달)
        → 서버에서 Gemini 호출
        → 카테고리 분류 + 중복 병합 결과 반환
    → 사이드패널 내 미리보기 화면으로 전환
    → 유저 확인 및 수정
    → "이대로 가져오기" 클릭
    → 서버에서 DB 중복 스킵 후 저장
    → 카테고리 → 컬렉션 자동 생성
...
AI가 23개를 4개 카테고리로 분류했어요.
수정하려면 항목을 드래그하거나 카테고리를 변경하세요.
...
| 액션            | 설명                               |
| --------------- | ---------------------------------- |
| 항목 제거       | `[×]` 클릭으로 특정 북마크 제외    |
| 카테고리 변경   | 항목의 카테고리를 다른 것으로 이동 |
| 카테고리명 수정 | 카테고리 제목 직접 편집            |
