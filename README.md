...
**[배포 링크](https://smartmark.wooyou.co.kr)** · **[기술 결정 문서](./docs/learning/decisions/)**
...
| 비회원 체험                 | localStorage 기반 10개 저장 + 대화 하루 5회 체험, 로그인 시 무제한   |
...
비회원은 localStorage 접근이 서버에서 불가하므로 prefetch 생략 → 기존 CSR 방식 그대로 유지. → [결정 문서](./docs/learning/decisions/022-ssr-tanstack-query-hydration.md)
...
헤더는 `initialized` 플래그가 `false`인 동안만 스켈레톤을 표시하고, 이후로는 리렌더 없이 일관된 UI를 유지한다. → [결정 문서](./docs/learning/decisions/020-zustand-global-auth-state.md)
...
한글 웹폰트(Pretendard) 도입 시 **통파일 방식과 `unicode-range` 동적 서브셋 방식을 모두 직접 구현**해 DevTools로 실측 비교했다. 그 결과 **폰트 전송량 2,058kB → 457kB (−78%)**, 페이지 전체 전송량을 37% 절감. CDN·셀프호스팅·`next/font`의 트레이드오프를 검토해 npm 셀프호스팅을 채택. → [결정 기록](./docs/learning/decisions/2026-06-07-webfont-dynamic-subset.md)
...
- **iOS Safari input 자동 줌 방지** — 모바일 폰트 16px 하한으로 포커스 시 확대 차단 ([결정 기록](./docs/learning/decisions/2026-06-07-ios-input-auto-zoom.md)).
