...
한글 웹폰트(Pretendard) 도입 시 **통파일 방식과 `unicode-range` 동적 서브셋 방식을 모두 직접 구현**해 DevTools로 실측 비교했다. 그 결과 **폰트 전송량 2,058kB → 457kB (−78%)**, 페이지 전체 전송량을 37% 절감. CDN·셀프호스팅·`next/font`의 트레이드오프를 검토해 npm 셀프호스팅을 채택. → [결정 기록](./docs/decisions/027-webfont-dynamic-subset.md)
...
- **iOS Safari input 자동 줌 방지** — 모바일 폰트 16px 하한으로 포커스 시 확대 차단 ([결정 기록](./docs/decisions/026-ios-input-auto-zoom.md)).
