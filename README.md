...
**[배포 링크](https://smartmark.wooyou.co.kr)** · **[Storybook](https://sohee-an.github.io/smart-bookmark-app/)** · **[기술 결정 문서](./docs/decisions/)**
...
모든 PR에서 **lint · typecheck · FSD 레이어 경계 · 단위/통합 테스트 · 빌드**를 병렬 실행하고, GitHub branch protection으로 **전부 통과해야 `dev`/`main`에 머지**된다. "테스트가 있다"가 아니라 "통과 못 하면 머지가 막힌다"를 보장. FSD 레이어 경계는 아키텍처 역방향 import를 `error`로 차단하는 전용 게이트다. → [개발 워크플로우](./docs/development-workflow.md)
...
- **Storybook + a11y 애드온** — 공용 UI·핵심 표시 컴포넌트를 상태별(BookmarkCard의 crawling/processing/completed/failed 등)로 문서화하고, `addon-a11y`로 접근성을 상시 점검. `main` 병합 시 GitHub Pages로 자동 게시([라이브](https://sohee-an.github.io/smart-bookmark-app/)).
...
### 결정론 경계 — 무엇을 강제하고 무엇을 사람에게 남기나

"AI를 얼마나 자동화했나"가 아니라 **"무엇을 강제하고 무엇을 온디맨드로 남길지"** 를 하나의 기준으로 나눴다 — **빠르고·객관적이고·결정론적인 것만 강제하고, 느리고·주관적이고·비싼 판단은 사람이 부를 때만 돌린다.** 모든 걸 강제하면 느려지고 개발자가 우회하기 때문이다.

| 시점        | 수단                    | 대상                                | 성격        |
| ----------- | ----------------------- | ----------------------------------- | ----------- |
| 파일 저장   | hook (`PostToolUse`)    | prettier · eslint --fix             | 강제·결정론 |
| PR 게이트   | CI required check       | typecheck · test · build · FSD 경계 | 강제·결정론 |
| 필요할 때   | 서브에이전트·CI 리뷰    | 코드리뷰 · 보안 · QA · UX           | 온디맨드    |
| 태스크 진행 | MCP 파이프라인 (Notion) | 상태 기반 개발 흐름                 | 트리거      |

- **저장 시 포맷 강제** — [`.claude/hooks/format.mjs`](./.claude/hooks/format.mjs)를 커밋해 팀이 동일 포맷 규칙을 공유. hook 입력을 stdin으로 파싱하고 편집 파일 기준으로 바이너리를 resolve해 Windows에서도 동작(`npx.cmd` EINVAL 회피).
- **FSD 레이어 경계를 CI에서 error로 강제** — 일반 lint는 `only-warn`이라 통과되므로, 아키텍처 역방향 import는 [별도 게이트](./packages/eslint-config/fsd.js)로 분리해 `--max-warnings 0`으로 막는다. 기계적 규칙은 CI가, 같은-레이어 판단은 온디맨드 `fsd-checker` 에이전트가 본다.
- **온디맨드 AI PR 리뷰** — [워크플로우](./.github/workflows/claude-review.yml)로 `ai-review` 라벨 또는 `@claude` 멘션 시에만 리뷰가 돈다(발신자 권한 게이트). 매 PR 강제가 아닌 이유는 LLM 호출이 느리고 비결정적이라서다. 로컬 에이전트와 **같은 커밋된 기준**(`code-review.md`)을 CI가 재사용한다.
- **컨텍스트 엔지니어링** — [`PROJECT_INDEX.md`](./PROJECT_INDEX.md)(키워드→파일 맵)로 AI 탐색을 전역 grep에서 핀포인트로 좁혀 컨텍스트 오염과 환각을 구조적으로 줄인다.

> 상세 설계와 근거: [AI 개발 환경 문서](./docs/ai-dev-environment.md)
...
- **Lint & Typecheck** — ESLint 규칙 + TypeScript strict 검사 + FSD 레이어 경계 게이트(역방향 import를 error로 차단)
