...
| 회원 저장              | Supabase                                           | ✅ 완료   |
| 태그 필터링            | 태그 클릭 → `/bookmarks?tag=React` 이동, 복수 선택 | ✅ 완료   |
| 키워드 검색            | 제목/설명 텍스트 매칭                              | ✅ 완료   |
...
```
[헤더] SmartMark  [검색창___________] [🏷 필터 (N)]  [+ 북마크 추가]
                   ↑ 글로벌 검색       ↑ /bookmarks에서만 표시

[본문 /bookmarks]
북마크 검색
[#React ×] [#Next.js ×]           ← 선택된 태그 뱃지 (X로 해제)
──────────────────────────────────────────────

결과 12개
│ 북마크 카드  [#React] [#Next.js] +2   ← +N 클릭 시 상세 패널 오픈
│ 북마크 카드
│ ...

# 시맨틱 검색 결과 (mode=semantic 시, 추후)
── 정확한 결과 ─────────────────
...
- 선택된 태그는 활성 스타일 (`bg-zinc-900 text-white`)
- 외부 클릭 시 닫힘
...
| 검색 종류   | 트리거                                                    | 동작                                                     |
| ----------- | --------------------------------------------------------- | -------------------------------------------------------- |
| 태그 필터   | 카드 태그 클릭 / 필터 버튼 드롭다운 / 상세 패널 태그 클릭 | URL `?tag=` 추가, 복수 선택(OR), 패널 자동 닫힘          |
| 키워드 검색 | 헤더 글로벌 검색창 입력 후 엔터                           | URL `?q=` 추가, 제목/요약 텍스트 매칭, 클라이언트 필터링 |
| 시맨틱 검색 | 검색창 입력 + 시맨틱 토글 ON (추후)                       | 벡터 유사도 기반 검색, 정확/연관 섹션 분리               |
...
components/
  layout/
    Header.tsx                          # 글로벌 검색창 + /bookmarks에서만 FilterBar 표시

pages/
  bookmarks.tsx                         # 쿼리스트링 읽기, 클라이언트 필터링, 선택 태그 뱃지

features/
  bookmark/
    ui/
      FilterBar.tsx                     # 필터 버튼 + 드롭다운 (최근/전체 태그, 복수 선택)
      TagFilter.tsx                     # 선택된 태그 뱃지 + X 제거 버튼
      SearchBar.tsx                     # (파일 존재, 현재 헤더 글로벌 검색으로 대체)
      BookmarkList.tsx                  # 결과 목록 렌더링 + empty state
      SemanticResultSection.tsx         # 정확한 결과 / 연관 결과 섹션 (추후)

entities/
  bookmark/
    ui/
      BookmarkCard.tsx                  # 태그 maxVisible=2, +N 뱃지 클릭 → 상세 패널
      BookmarkDetailPanel.tsx           # 뷰 모드 태그 클릭 → 필터 이동 + 패널 닫힘

shared/
  ui/
    tag/Tag.tsx                         # TagGroup: onTagClick, onMoreClick(+N 뱃지) 지원
```
...
| 카드 태그                | 필터 추가 (`?tag=`)       |
| 카드 `+N` 뱃지           | 상세 패널 오픈            |
| 필터 드롭다운 태그       | 필터 추가/해제 토글       |
| 상세 패널 태그 (뷰 모드) | 패널 닫힘 + 필터 이동     |
| 메인(/) 카드/패널 태그   | `/bookmarks?tag=` 로 이동 |
